...
const LandlordsListScreen = ({ 
  landlordsList, 
  likedLandlords: externalLikedLandlords, // Проп для Обраного
  onBack, 
  onOpenDetails, 
  onToggleFavorite, 
  onLogoClick 
}) => {
  const { likedLandlords: contextLikedLandlords } = useFavorites();
  const [activeTab, setActiveTab] = useState('home');

  // Визначаємо, який список лайків використовувати
  const likedData = externalLikedLandlords || contextLikedLandlords;

// У LandlordsListScreen.js
const isLiked = (id) => {
    ...
    <div className="relative w-full h-full flex flex-col font-montserrat bg-white overflow-hidden">
      ...
      <div className="relative z-10 flex flex-col flex-1 min-h-0 pb-28" style={{ overflowY: 'auto', scrollbarWidth: 'none' }}>
        ...
        <div className="flex items-center gap-3 px-6 pt-9 pb-6">
          ...
          <h1 className="font-bold text-[32px] leading-[100%] text-[#012A81]">Орендодавці</h1>
        </div>

        {/* СІТКА */}
        {landlordsList.length === 0 ? (
          <div className="flex-1 flex items-center justify-center px-6">
            <p className="text-[#8a9ab8] text-[16px] font-medium text-center">Список орендодавців порожній</p>
          </div>
        ) : (
  ...
  <LandlordCard 
    ...
    isLiked={isLiked(landlord.id)} // <--- ТУТ має бути id
...
export default LandlordsListScreen;