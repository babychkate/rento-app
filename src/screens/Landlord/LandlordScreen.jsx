...
// Перевір правильність шляху до папки context
...
const ArrowRightIcon = () => (
  <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
    <path d="M8 4L16 12L8 20" stroke="#0052FF" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
  </svg>
);
      ...
      <div className="flex items-center gap-2 mb-3">
        <div className="flex gap-0.5">
          {Array.from({ length: 5 }).map((_, i) => (
            <svg key={i} width="13" height="13" viewBox="0 0 24 24" fill={i < review.rating ? '#8F94FB' : '#e2e8f0'}>
...
const LandlordScreen = ({ property, onBack }) => {
  ...
  // Отримуємо колекцію likedLandlords та функцію перемикання з нашого контексту
  const { likedLandlords, toggleLandlord } = useFavorites();

  const landlord = property?.landlord;
  
  // Перевіряємо через метод .has(), чи є ID орендодавця в нашому Set обраного
  const isLiked = landlord?.id ? likedLandlords.has(landlord.id) : false;
  ...
  return (
    <div className="relative w-full h-full flex flex-col font-montserrat bg-[#f1f2f6]">
      <div className="flex-1 min-h-0 pb-28" style={{ overflowY: 'auto', overflowX: 'hidden', scrollbarWidth: 'none' }}>
        
        {/* TOP BAR */}
        <div className="flex items-center justify-between px-6 pt-14 pb-4">
          <button onClick={onBack} className="bg-transparent border-none cursor-pointer p-1">
            <svg width="24" height="24" viewBox="0 0 24 24" fill="none">
              <path d="M16 20L8 12L16 4" stroke="#012A81" strokeWidth="3" strokeLinecap="round" strokeLinejoin="round"/>
            </svg>
          </button>
        ...
        <div className="mx-6 rounded-[28px] overflow-hidden relative h-[300px] shadow-[0_8px_28px_rgba(0,30,140,0.18)]">
          <img src={landlord?.avatar} alt={landlord?.name} className="w-full h-full object-cover object-top" />
          ...
          <button 
            onClick={(e) => {
              ...
              e.preventDefault();
              if (landlord?.id) {
                toggleLandlord(landlord.id); // ВИПРАВЛЕНО: передаємо суто ID
                console.log("✅ Стан змінено для орендодавця з ID:", landlord.id);
              }
            }}
            className="absolute bottom-[22px] right-6 z-50 bg-transparent border-none cursor-pointer p-2 flex items-center justify-center active:scale-95 transition-transform"
          ...
          {/* Ім'я */}
          <span className="absolute bottom-[18px] left-5 font-bold text-[26px] text-white" style={{ textShadow: '0 2px 8px rgba(0,20,100,0.4)' }}>
        ...
        <div className="flex items-start justify-between gap-3 px-6 pt-5">
          <ul className="flex-1 flex flex-col gap-1.5 list-none">
            {[
              ...
              'Високий уровень довіри серед орендарів',
            ].map(item => (
        ...
        <div className="flex flex-col px-6">
          {[
            { icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#0052FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M6.62 10.79a15.05 15.05 0 0 0 6.59 6.59l2.2-2.2a1 1 0 0 1 1.01-.24c1.12.37 2.33.57 3.58.57a1 1 0 0 1 1 1V20a1 1 0 0 1-1 1A17 17 0 0 1 3 4a1 1 0 0 1 1-1h3.5a1 1 0 0 1 1 1c0 1.25.2 2.45.57 3.58a1 1 0 0 1-.25 1.01l-2.2 2.2z"/></svg>, value: '+380715829471' },
            { icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#0052FF" strokeWidth="1.5"><rect x="2" y="2" width="20" height="20" rx="5"/><circle cx="12" cy="12" r="4"/><circle cx="17.5" cy="6.5" r="1" fill="#0052FF" stroke="none"/></svg>, value: '@nadiia.yukhymchuk' },
            { icon: <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#0052FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21.5 4.5L2.5 11.5L9.5 13.5M21.5 4.5L9.5 13.5M21.5 4.5L14.5 20.5L9.5 13.5M9.5 13.5V19.5L12.5 16.5"/></svg>, value: '@nadiia_rento' }
        ...
        <div className="flex flex-col px-6">
          {[
            { key: 'identity', icon: <svg width="26" height="26" viewBox="0 0 42 42" fill="none"><path d="M35.2912 9.97429C34.8714 9.45643 34.1227 8.6615 33.6489 8.19351C30.3979 4.98235 25.9305 3 21 3C11.0589 3 3 11.0589 3 21C3 30.9411 11.0589 39 21 39C30.9411 39 39 30.9411 39 21C39 18.8672 38.6291 16.821 37.9482 14.9225" stroke="#0052FF" strokeWidth="2" strokeLinecap="round"/><path d="M13.1514 22.353L16.7953 27.0018C17.1654 27.474 17.8651 27.5162 18.2894 27.092L35.292 10.0894" stroke="#0052FF" strokeWidth="2" strokeLinecap="round"/></svg>, label: 'Особу підтверджено' },
            { key: 'security', icon: <svg width="26" height="26" viewBox="0 0 42 42" fill="none"><path d="M37.824 8.71191L21.6457 2.14918C21.4095 2.05336 21.1456 2.05136 20.908 2.14359L4.72551 8.4243C4.29883 8.5899 4.03904 9.02404 4.09477 9.47833L6.15839 26.2978C6.35645 27.912 7.20129 29.3767 8.49943 30.3564L20.6756 39.5454C21.0321 39.8145 21.5238 39.8145 21.8803 39.5454L34.0565 30.3564C35.3546 29.3767 36.1995 27.912 36.3975 26.2978L37.796 14.8997" stroke="#0052FF" strokeWidth="2" strokeLinecap="round"/><path d="M12.3701 22.7123L16.1516 27.0389C16.5183 27.4585 17.1573 27.4974 17.5722 27.1253L38 8.80521" stroke="#0052FF" strokeWidth="2" strokeLinecap="round"/></svg>, label: <><span className="text-[#0052FF] font-bold">RENTO</span> розробив комплексну безпеку</> }
            ...
            <div key={key}>
              <div className="flex items-center justify-between py-3.5">
                ...
                <button onClick={() => setVerificationScreen(key)} className="bg-transparent border-none cursor-pointer p-1 ml-2 flex-shrink-0">
                  <ArrowRightIcon />
                </button>
              </div>
              {idx < arr.length - 1 && <div className="h-px bg-[rgba(41,121,255,0.1)]" />}
        ...
        <div className="flex gap-3 px-6 pt-3 pb-4" style={{ overflowX: 'auto', scrollbarWidth: 'none' }}>
          {reviews.map(review => (
