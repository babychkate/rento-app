...
import { useFavorites } from '../../screens/Context/FavouritesContext'; // Перевір правильність шляху до папки context
...
const RoommateScreen = ({ roommate, onBack }) => {
  const [verificationScreen, setVerificationScreen] = useState(null);
  
  // Підключаємо твій контекст обраного
  const { isRoommateLiked, toggleRoommate } = useFavorites();

  // Надання дефолтних значень, якщо пропси не передані (fallback з HTML-шаблону)
  const currentRoommate = roommate ?? {
    id: 'r1', // Додали id для коректної роботи контексту при дефолтних значеннях
    name: 'Олена',
    avatar: 'https://images.unsplash.com/photo-1534528741775-53994a69daeb?auto=format&fit=crop&w=800&q=80',
    rating: '4.9',
    phone: '+380631234567',
    instagram: '@olena.shvets',
    telegram: '@olena_rento',
  };

  // Визначаємо, чи лайкнутий цей конкретний співмешканець
  const isLiked = currentRoommate.id ? isRoommateLiked(currentRoommate.id) : false;

  // Відгуки про співмешканця
  const reviews = roommate?.reviews ?? [
    {
      id: 'r-r1',
      name: 'Максим (Орендодавець)',
      avatar: 'https://i.pravatar.cc/80?img=11',
      rating: 5,
      date: '3 дні тому',
      text: "Чудова співмешканка! Завжди підтримує чистоту в спільних зонах, вчасно закриває рахунки за комуналку. Комунікація дуже легка...",
    },
    {
      id: 'r-r2',
      name: 'Дмитро (Екс-сусід)',
      avatar: 'https://i.pravatar.cc/80?img=47',
      rating: 5,
      date: '1 місяць тому',
      text: "Жили разом в трикімнатній квартирі пів року. Олена дуже спокійна, поважає особистий простір, ніякого галасу. Дуже рекомендую!",
    },
  ];
    ...
    <div className="relative w-full h-full flex flex-col font-montserrat bg-[#f1f2f6]">
      ...
      <div className="flex-1 min-h-0 pb-28"
        ...
        <div className="mx-6 rounded-[28px] overflow-hidden relative h-[300px] shadow-[0_8px_28px_rgba(0,30,140,0.18)]">
          ...
          {/* Чисте біле сердечко без кружечків та зайвих кольорів */}
          <button 
            onClick={(e) => {
              e.stopPropagation();
              if (currentRoommate.id) {
                toggleRoommate(currentRoommate.id);
              }
            }}
            className="absolute bottom-[22px] right-6 z-10 bg-transparent border-none cursor-pointer p-0 flex items-center justify-center active:scale-90 transition-transform"
          >
            <svg 
              width="26" 
              height="24" 
        ...
        <div className="flex items-start justify-between gap-3 px-6 pt-5">
          <ul className="flex-1 flex flex-col gap-1.5 list-none">
            {[
              'Перевірений співмешканець',
              '3 роки користується платформою',
          ...
          <div className="flex-shrink-0 flex items-center gap-1.5 bg-[#3173FD] rounded-xl px-3 py-1.5 mt-0.5">
            ...
            <span className="text-white text-[13px] font-medium">{currentRoommate.rating}</span>
          </div>
        </div>
        ...
        <div className="flex flex-col px-6">
          {[
            ...
            },
            {
              ...
              value: currentRoommate.instagram,
            },
            {
              icon: (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#0052FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
              ...
              value: currentRoommate.telegram,
            },
