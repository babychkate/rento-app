...
const ROOMMATE_REVIEWS = [
  {
    ...
    rating: 5,
    date: '3 дні тому',
    text: 'Чудовий співмешканець! Завжди підтримує чистоту в спільних зонах, вчасно закриває рахунки за комуналку. Комунікація дуже легка та приємна.',
  },
  {
    id: 'rm-r2',
    ...
    rating: 5,
    date: '1 місяць тому',
    text: 'Жили разом у трикімнатній квартирі пів року. Дуже спокійний, поважає особистий простір, ніякого галасу. Дуже рекомендую!',
  },
];
...
const ReviewCard = ({ review }) => {
  ...
  return (
    <div
      className="flex-shrink-0 w-[295px] rounded-3xl p-[18px] border border-white/80"
      style={{
...
const RoommateScreen = ({ roommate, onBack }) => {
  ...
  // ДОДАНО: Стан для керування вкладеними екранами верифікації
  const [verificationScreen, setVerificationScreen] = useState(null);
  
  const { likedRoommates, toggleRoommate } = useFavorites();

  const isLiked = roommate?.id ? likedRoommates.has(roommate.id) : false;

  // ДОДАНО: Пріоритетний рендер екранів верифікації, якщо стан не null
          ...
          {[
            {
              icon: (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
                  stroke="#0052FF" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round">
            ...
            },
            {
              ...
              value: `@${roommate?.name?.toLowerCase().replace(' ', '.') ?? 'roommate'}`,
            },
            {
              icon: (
                <svg width="24" height="24" viewBox="0 0 24 24" fill="none"
              ...
              value: `@${roommate?.name?.toLowerCase().replace(' ', '_') ?? 'roommate'}_rento`,
            },
        ...
        <div className="flex flex-col px-6">
          {[
            ...
            },
            {
              ...
              label: <><span className="text-[#0052FF] font-bold">RENTO</span> розробив комплексну безпеку</>,
            },
            ...
            <div key={key}>
              <div className="flex items-center justify-between py-3.5">
                ...
                {/* ВИПРАВЛЕНО: Додано onClick для запису ключа верифікації в стейт */}
                <button 
                  onClick={() => setVerificationScreen(key)} 
